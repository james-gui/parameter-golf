...
class Hyperparameters:
    ...
    warmdown_iters = int(os.environ.get("WARMDOWN_ITERS", 370))
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self._qat and self.training and w.ndim == 2:
            ...
            scale = amax / 31.0
            q = (w_f / scale).round().clamp(-31, 31) * scale
            w = w + (q - w_f).detach()  # STE: forward sees quantized, backward sees original
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.use_xsa:
            # Subtract self-value projection (GQA-aware)
            B, Hq, T, D = y.shape
            Hkv = v.size(1)
            group = Hq // Hkv
...
def main() -> None:
    ...
    for module in base_model.modules():
        if isinstance(module, CastedLinear):
            module.float()
            module._qat = True  # Enable int6 STE QAT
